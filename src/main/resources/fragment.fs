...
{
	vec3 colour;
	vec3 position; // in mv Coordianates
	float intensity;
	Attenuation att;
};
...
vec4 calcLightColour(vec3 light_colour, float light_intensity, vec3 position, vec3 to_light_dir, vec3 normal) {
	...
	// Specular light
	vec3 camera_direction = normalize(cameraPos - position);
...
vec4 calcPointLight(PointLight light, vec3 position, vec3 normal) {
	...
	vec4 light_colour = calcLightColour(light.colour, light.intensity, position, to_light_source, normal);
